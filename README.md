...
- HLS の**ライブ録画**
- 検出した動画ごとの**サムネイル表示**（ページの再生フレーム / poster / og:image から生成）
- 複数メディアの**一括保存**
...
保存時にも同じ権限が必要です。許可していない配信元のメディアを保存しようとすると、失敗理由として**そのホスト名**が表示され、ジョブ一覧に「配信元を許可して再試行」ボタンが出ます。ボタンを押すと必要なホストだけを許可し、同じ保存をそのまま再開します（メディアを探し直す必要はありません）。
...
- HLS / ライブ録画の FFmpeg 出力（リマックス・録画）: 上限なし。マルチプレクサの書き込みをそのまま OPFS ファイルへ流し、完成物はディスク上のファイルのまま Downloads へ渡すため、RAM 使用量は出力サイズに比例しません（ディスク容量のみが上限）。
- OPFS を使うセグメント結合・DASH 単一トラック組み立て: **768 MiB**
- DASH の映像 + 音声を FFmpeg で多重化する場合: **合計入力 384 MiB**
- FFmpeg 出力の書き込み待ちキュー: **512 MiB**（ファイルシステムが追いつかない場合のみ発動し、その時点で明示的に失敗）
